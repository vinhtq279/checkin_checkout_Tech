...
/******* Ve som bat duoc tu quet the ******************/
  $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() -1 ) and t.TrDate < convert(date,getdate()) and t.TrTime < '17:15:00' and t.TrTime >= '16:00:00' order by t.TrDate, t.TrTime";
...
/************ Them nhung thang ve som ma ko quet the ma bat dc tu camera *************************/
  $querymysql = "select l.user_id as userID, l.updated_at as TrDateTime, e.card_num as CardNo, c.id as CameraID, c.name as CameraName, p.full_name as TrName from log_dossier l, employee e, camera c, person p  where l.user_id = e.user_id and l.camera_id = c.id and p.id = l.user_id and c.id = 1 and l.updated_at >= date_add(date_sub(curdate(), interval 1 day), interval '16:00' HOUR_MINUTE) and l.updated_at < date_add(date_sub(curdate(), interval 1 day), interval '17:15' HOUR_MINUTE) order by l.updated_at desc"; //camera cua chinh ra - camera.id = 1
  $retmysql = mysqli_query($conmysql,$querymysql);
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   $checkquery = "select count(*) as allcount from vesom where CardNo = $rowmysql[CardNo]";
   $retmysql2=mysqli_query($conmysql,$checkquery);
   $rowmysql2=mysqli_fetch_array($retmysql2);
   $allcount = $rowmysql2['allcount'];
   if($allcount == 0){
    $querysqlsrv = "select distinct t.TrName, c.CardDB_09 as Depart from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo like '%" . $rowmysql[CardNo] . "%' ";
    //$querysqlsrv = "select t.TrName, t.CardNo, c.CardDB_09 as Depart from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo like '%" . $rowmysql[CardNo] . "%' ";
    $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
    while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
     $querymysql = "insert into vesom (CardNo, Depart, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowsqlsrv[Depart]', '$rowsqlsrv[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
...
/************************* Insert vemuon table from card data**********************************/
  ...
  $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() - 1 ) and t.TrDate < convert(date,getdate()) and t.TrTime >= '17:15:00' and t.TrTime <= '21:00:00' order by t.TrDate, t.TrTime";
  $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
  while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
...
/************ Them nhung thang ve muon ma ko quet the ma bat dc tu camera *************************/
  $querymysql = "select l.user_id as userID, l.updated_at as TrDateTime, e.card_num as CardNo, c.id as CameraID, c.name as CameraName, p.full_name as TrName from log_dossier l, employee e, camera c, person p  where l.user_id = e.user_id and l.camera_id = c.id and p.id = l.user_id and c.id = 1 and l.updated_at >= date_add(date_sub(curdate(), interval 1 day), interval '17:15' HOUR_MINUTE) and l.updated_at <= date_add(date_sub(curdate(), interval 1 day), interval '21:00' HOUR_MINUTE) order by l.updated_at desc"; //camera cua chinh ra - camera.id = 1
  $retmysql = mysqli_query($conmysql,$querymysql);
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   $checkquery = "select count(*) as allcount from vemuon where CardNo = $rowsqlsrv[CardNo]";
   $retmysql2=mysqli_query($conmysql,$checkquery);
   ...
   if($allcount == 0){ // Khong co trong vemuon table{
    ...
    $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
    while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
     $querymysql = "insert into vemuon (CardNo, Depart, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowsqlsrv[Depart]', '$rowsqlsrv[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
     $ret = mysqli_query($conmysql,$querymysql);
    }
   }
  }
/************ Loai nhung thang ve som xong quay lai ma co trong bang ve som ******************/
  ...
  $retmysql = mysqli_query($conmysql,$querymysql);
?>
  <h1>Danh sách cán bộ có con tan học sớm, nhà nhiều việc</h1>
  <table>
   <th>Số thẻ</th>
   <th>Quý Danh</th>
...
<?php
  while ($rowmysql=mysqli_fetch_array($retmysql)) {?>
    ...
    <td><?php echo $rowmysql['Camera'];?></td>
    </tr>
<?php
  }
?>
