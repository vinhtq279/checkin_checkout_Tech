...
/************ Them nhung thang di som ma ko quet the ma bat dc tu camera *************************/
  ...
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   $checkquery = "select count(*) as allcount from disom where CardNo = $rowmysql[CardNo]";
   $retmysql2=mysqli_query($conmysql,$checkquery);
   $rowmysql2=mysqli_fetch_array($retmysql2);
   $allcount = $rowmysql2['allcount'];
   if($allcount == 0){
    //$querysqlsrv = "select distinct TrName from tblTransaction where CardNo like '%" . $rowmysql[CardNo] . "%' ";
    $querysqlsrv = "select distinct t.TrName, t.CardNo, c.CardDB_09 as Depart from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo like '%" . $rowmysql[CardNo] . "%' ";
    $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
    while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
     $querymysql1 = "insert into som (CardNo, Depart, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowsqlsrv[Depart]', '$rowsqlsrv[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
     $ret = mysqli_query($conmysql,$querymysql1);
    }
   }
  }
...
/************************* Insert dimuon table from card data**********************************/
  ...
  $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() - 1 ) and t.TrDate < convert(date,getdate()) and t.TrTime > '08:00:00' and t.TrTime <= '10:00:00' order by t.TrDate, t.TrTime";
  $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
  while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
   $checkquery = "select count(*) as allcount from disom where CardNo = $rowsqlsrv[CardNo]";
   $retmysql=mysqli_query($conmysql,$checkquery);
...
/************ Them nhung thang di muon ma ko quet the ma bat dc tu camera *************************/
  $querymysql = "select l.user_id as userID, l.updated_at as TrDateTime, e.card_num as CardNo, c.id as CameraID, c.name as CameraName, p.full_name as TrName from log_dossier l, employee e, camera c, person p  where l.user_id = e.user_id and l.camera_id = c.id and p.id = l.user_id and c.id = 2 and l.updated_at > date_add(date_sub(curdate(), interval 1 day), interval '08:00' HOUR_MINUTE) and l.updated_at <= date_add(date_sub(curdate(), interval 1 day), interval '10:00' HOUR_MINUTE) order by l.updated_at desc"; //camera cua chinh vao - camera.id = 2
  $retmysql = mysqli_query($conmysql,$querymysql);
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   $checkquery = "select count(*) as allcount from dimuon where CardNo = $rowmysql[CardNo]";
   $retmysql2=mysqli_query($conmysql,$checkquery);
   $rowmysql2=mysqli_fetch_array($retmysql2);
   $allcount = $rowmysql2['allcount'];
   echo $rowmysql[CardNo];
   if($allcount == 0){ // Khong co trong dimuon table
    $querysqlsrv = "select distinct t.TrName, t.CardNo, c.CardDB_09 as Depart from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo like '%" . $rowmysql[CardNo] . "%' ";
    $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
    while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
    $querymysql1 = "insert into dimuon (CardNo, Depart, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowsqlsrv[Depart]', '$rowsqlsrv[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
     $ret = mysqli_query($conmysql,$querymysql1);
    }
   }
  }
...
/************ Loai nhung thang di muon ma co trong bang di som ******************/
  ...
  $retmysql = mysqli_query($conmysql,$querymysql);
?>
  <h1>Danh sách cán bộ bị tắc đường, hỏng xe, ăn sáng muộn</h1>
  <table>
   <th>Số thẻ</th>
   <th>Quý Danh</th>
