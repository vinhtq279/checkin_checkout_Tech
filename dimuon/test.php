...
<input type="submit" name="filter" value="chơi" style="width:150px; background-color:Brown">
<input type='text' placeholder='don vi' name='depart'></input>
<?php
...
/******* Di som bat duoc tu quet the ******************/
  if($_POST[depart]){
   echo $_POST[depart];
   $retsqlsrv = sqlsrv_query($consqlsrv, "delete from dimuon");
   $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() -1 ) and t.TrDate < convert(date,getdate()) and t.TrTime <= '08:00:00' and t.TrTime >= '05:00:00' and c.CardDB_09 like '%" . $_POST[depart] . "%' order by t.TrDate, t.TrTime";
  }else{
...
/************ Them nhung thang di som ma ko quet the ma bat dc tu camera *************************/
  ...
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   ...
   $allcount = $rowmysql['allcount'];
   if($allcount == 0){
    $querymysql = "insert into som (CardNo, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowmysql[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
...
/************************* Insert dimuon table from card data**********************************/
  ...
  if($_POST[depart]){
   $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() - 1 ) and t.TrDate < convert(date,getdate()) and t.TrTime > '08:00:00' and t.TrTime <= '11:00:00' and order by t.TrDate, t.TrTime";
  }else{
   $querysqlsrv = "select t.CardNo as CardNo, c.CardDB_09 as Depart, t.TrName as TrName, convert(varchar,convert(datetime,(t.TrDate + ' ' + t.TrTime ),20),20) as TrDateTime from tblTransaction t, CardDB c where t.CardNo = c.CardNo and t.CardNo not like '%FFFF%' and t.TrDate >= convert(date,getdate() - 1 ) and t.TrDate < convert(date,getdate()) and t.TrTime > '08:00:00' and t.TrTime <= '11:00:00' and c.CardDB_09 like '%" . $_POST[depart] . "%' order by t.TrDate, t.TrTime";}
  $retsqlsrv = sqlsrv_query($consqlsrv, $querysqlsrv);
  while($rowsqlsrv = sqlsrv_fetch_array($retsqlsrv)) {
   $checkquery = "select count(*) as allcount from disom where CardNo = $rowsqlsrv[CardNo]";
   $retmysql=mysqli_query($conmysql,$checkquery);
...
/************ Them nhung thang di muon ma ko quet the ma bat dc tu camera *************************/
  ...
  while ($rowmysql=mysqli_fetch_array($retmysql)) {
   ...
   $allcount = $rowmysql['allcount'];
   if($allcount == 0){ // Khong co trong dimuon table
    $querymysql = "insert into dimuon (CardNo, TrName, TrDateTime, Camera) values ($rowmysql[CardNo], '$rowmysql[TrName]', '$rowmysql[TrDateTime]', 'Camera')";
